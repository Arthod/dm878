...
with open("traffic_accidents_2018.csv", "r") as f:
    for i, line in enumerate(f.readlines()):
        ...
        hour = datetime.strftime(
            datetime.fromtimestamp(
                int(datetime.timestamp(datetime.strptime(t, FORMAT_FROM)) % (24 * 60 * 60)) + 315529200
                ), FORMAT_TO)
        
        week = datetime.strftime(
            datetime.fromtimestamp(
                int(datetime.timestamp(datetime.strptime(t, FORMAT_FROM)) % (24 * 60 * 60 * 7)) + 315529200
                ), FORMAT_TO)

        month = datetime.strftime(
            datetime.fromtimestamp(
                int(datetime.timestamp(datetime.strptime(t, FORMAT_FROM)) % (24 * 60 * 60 * 30)) + 315529200
                ), FORMAT_TO)
